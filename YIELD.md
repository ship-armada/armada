...
| Lend (USDC â†’ yield) | Zero |
| Redeem (yield â†’ USDC) | 15% of yield |
| Pay from Yield (direct) | 15% of yield |
...
```
                ...
                â”‚           UNSHIELDED WORLD              â”‚
                â”‚                                         â”‚
                â”‚   [USDC]                                â”‚
                â”‚    â”‚ â”‚ â–²                                â”‚
                â””â”€â”€â”€â”€â”¼â”€â”¼â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                     â”‚ â”‚ â”‚
           shield to â”‚ â”‚ â”‚ unshield
               yield â”‚ â”‚ â”‚ + pay from yield
                     â”‚ â”‚ â”‚
                â”Œâ”€â”€â”€â”€â”¼â”€â”¼â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
                â”‚    â”‚ â–¼ â”‚                                â”‚
                â”‚    â”‚[Shielded    lend     [Shielded     â”‚
                â”‚    â”‚ USDC] â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â†’  Yield]â”€â”€â”€â”€â”€â”€â”€â”¤
                â”‚    â”‚  â–² â”‚                    â”‚          â”‚
                â”‚    â”‚  â”‚ â”‚      redeem        â”‚          â”‚
...
| Redeem | Shielded Yield | Shielded USDC | 15% of yield |
| Redeem + Unshield | Shielded Yield | Unshielded USDC | 15% of yield |
| Redeem + Unshield (CCTP) | Shielded Yield | USDC (other chain) | 15% of yield |
...
```
USDC (Arbitrum) â†’ shield to yield via CCTP â†’ Shielded Yield
```

**Pay directly from yield:**
```
Shielded Yield â†’ redeem + unshield via CCTP â†’ Recipient (Base)
```

**Deposit, earn, then pay:**
```
USDC â†’ shield â†’ Shielded USDC â†’ lend â†’ Shielded Yield
  ... time passes ...
Shielded Yield â†’ redeem â†’ Shielded USDC â†’ unshield â†’ Recipient
```

**Send yield position to friend:**
```
Shielded Yield â†’ transfer â†’ Friend's Shielded Yield
```

**Exit everything:**
```
Shielded Yield â†’ redeem + unshield â†’ USDC
```
...
```
User USDC â†’ ArmadaYieldVault â†’ Aave v4 Vault â†’ Lending Pool
                â†“
...
Aave v4 uses ERC-4626 share accountingâ€”share balances stay constant while share price increases. This is essential for compatibility with shielded notes, which commit to fixed token amounts.

```
Time 0: User lends 1000 USDC â†’ receives 1000 shares @ $1.00
Time 1: Aave generates 6% yield â†’ 1000 shares now worth $1060
...
Unshielded USDC â†’ Shielded USDC.
...
Unshielded USDC â†’ Shielded Yield in one step.
...
Convert shielded yield position to shielded USDC. 15% yield fee.
...
Shielded USDC â†’ Unshielded USDC. Zero fee.

Standard Railgun unshield operation.

### Redeem + Unshield (Direct Payment from Yield)

Shielded Yield â†’ Unshielded USDC in one step. 15% yield fee.
...
Shielded Yield â†’ USDC on another chain in one step. 15% yield fee.
...
contract ArmadaYieldVault is ERC4626 {
    ...
    uint256 public constant YIELD_FEE_BPS = 1500; // 15%
...
- Armada captures value from yield (15%), not from internal churn
...
1. Shield fees (one-time):
   $1M deposits Ã— 0.5% = $5,000

2. Yield fees (recurring):
   $10M in vault Ã— 5% APY Ã— 15% fee = $75,000/year
```
...
| Fee on lend/redeem | Zero (15% of yield on redeem) |
